...
class TrainingSettings:
    ...
    optuna_validation_fraction: float = 0.1
    ...
    walk_forward_gates: Dict[str, Any] = field(default_factory=dict)
    walk_forward_stop_on_fail: bool = True
    walk_forward_warm_start: bool = False
    warm_start_model_path: Optional[str] = None
    walk_forward_full_window: bool = False
    walk_forward_window: Optional[Dict[str, Any]] = None
    walk_forward_gate_grace_windows: int = 0
    ...
    def validate(self) -> List[str]:
        ...
        intraday_mode = bool(self.intraday_enabled) or (self.data_frequency or "").lower() in {"intraday", "15m", "15min"}
        if self.optuna_trials not in (None, 0):
            min_trials = 1 if intraday_mode else 10
            if self.optuna_trials < min_trials:
                errors.append(f"Optuna trials must be at least {min_trials}")

        if not 0.0 <= float(self.optuna_validation_fraction) <= 0.5:
            errors.append("optuna_validation_fraction must be within [0, 0.5]")

        gates = self.walk_forward_gates or {}
        if gates:
            for key in ("min_trades", "min_sharpe"):
                value = gates.get(key)
                if value not in (None, ""):
                    try:
                        numeric = float(value)
                    except (TypeError, ValueError):
                        errors.append(f"walk_forward_gates.{key} must be numeric when provided")
                        continue
                    if numeric < 0:
                        errors.append(f"walk_forward_gates.{key} must be non-negative")
            dd_value = gates.get("max_drawdown")
            if dd_value not in (None, ""):
                try:
                    dd_numeric = float(dd_value)
                except (TypeError, ValueError):
                    errors.append("walk_forward_gates.max_drawdown must be numeric when provided")
                else:
                    if dd_numeric <= 0:
                        errors.append("walk_forward_gates.max_drawdown must be positive")
        ...
        if not isinstance(self.walk_forward_full_window, bool):
            errors.append("walk_forward_full_window must be a boolean value")

        if self.walk_forward_window not in (None, {}):
            if not isinstance(self.walk_forward_window, dict):
                errors.append("walk_forward_window must be a dict when provided")
            else:
                required_keys = {"train_start", "train_end", "test_start", "test_end"}
                missing = required_keys - set(self.walk_forward_window.keys())
                if missing:
                    errors.append(
                        "walk_forward_window must include train_start, train_end, test_start, test_end"
                    )
                else:
                    for key in required_keys:
                        value = self.walk_forward_window.get(key)
                        if not isinstance(value, str):
                            errors.append(
                                f"walk_forward_window.{key} must be an ISO date string"
                            )
                            break

        try:
            grace_windows = int(self.walk_forward_gate_grace_windows)
            if grace_windows < 0:
                raise ValueError
            self.walk_forward_gate_grace_windows = grace_windows
        except (TypeError, ValueError):
            errors.append("walk_forward_gate_grace_windows must be a non-negative integer")
...
class TrainingConfig:
    ...
    def __post_init__(self):
        ...
        if self.agent_type == 'SAC_INTRADAY_DSR':
            ...
            if not self.training_settings.max_total_timesteps:
                self.training_settings.max_total_timesteps = max(200_000, self.training_settings.total_timesteps)
            elif self.training_settings.max_total_timesteps < self.training_settings.total_timesteps:
                self.training_settings.max_total_timesteps = self.training_settings.total_timesteps
